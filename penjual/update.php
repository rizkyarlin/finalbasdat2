<?php
	...
	if($_SERVER['REQUEST_METHOD'] === 'GET'){
		$query = "SELECT * FROM penjual WHERE id=" . $_GET['id'];
		$rs = $db->query($query);
		$row = $rs->fetch_row();
	}
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		require('../database/connect.php');
		$stmt = $db->prepare("UPDATE penjual SET 
								kode_penjual = ? 
								, nama = ?
								, umur = ?
								, hp = ?
								...
								WHERE id = ?
								");
		$stmt->bind_param('ssssss', $_POST['kode_penjual'], $_POST['nama'], $_POST['umur'], $_POST['hp'], $_POST['alamat'], $_POST['id']);
...
<div class="container bg-info">
	<div class="row">
		...
		<h1 class="text-center bg-primary">Update Penjual</h1>
			<div class="col-md-6 col-md-offset-3">
				<form name="formPenjual" id="formPenjual" method="post" action="">
					<div class="form-group">
						<label for="kode_penjual">Kode Penjual</label>
						<input type="text" class="form-control"  name = "kode_penjual" id="kode_penjual"  value="<?=($row[1]);?>" required>
					</div>
					<div class="form-group">
